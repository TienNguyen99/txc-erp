...
@section('content')
                    ...
                    <button class="nav-link" id="perm-tab" data-bs-toggle="tab" data-bs-target="#tab-perm" type="button">
                        ...
                        @if (!empty($userDirectPermissions))
                            <span class="badge bg-primary ms-1"
                                style="font-size:.7rem">{{ count($userDirectPermissions) }}</span>
                        @endif
                    </button>
                </li>
            </ul>
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="role" class="form-select @error('role') is-invalid @enderror" required>
                                    ...
                                    <option value="staff" {{ $currentRole === 'staff' ? 'selected' : '' }}>Staff — Nhân
                                        viên kho</option>
                                    <option value="manager" {{ $currentRole === 'manager' ? 'selected' : '' }}>Manager —
                                        Quản lý (cần cấp quyền)</option>
                                    <option value="admin" {{ $currentRole === 'admin' ? 'selected' : '' }}>Admin — Toàn
                                        quyền</option>
                                </select>
                                <div class="form-text">Role cấp quyền mặc định theo nhóm. Quyền riêng có thể tuỳ chỉnh ở tab
                                    bên.</div>
                                @error('role')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                    ...
                    <div class="card-page" style="border-radius:0 8px 8px 8px">
                        <div class="d-flex align-items-center justify-content-between mb-3">
                            <div>
                                <h6 class="mb-0 fw-semibold"><i
                                        class="fa-solid fa-shield-halved me-2 text-primary"></i>Quyền riêng cho
                                    <strong>{{ $user->name }}</strong></h6>
                                <small class="text-muted">Quyền riêng này <em>cộng thêm</em> vào role hiện tại. Admin có
                                    toàn quyền mặc định.</small>
                            </div>
                        ...
                        <form method="POST" action="{{ route('admin.users.sync-permissions', $user) }}" id="permForm">
                                            ...
                                            <tr>
                                                <td class="fw-semibold">
                                                    <span class="badge bg-light text-dark border me-1"
                                                        style="font-size:.7rem">{{ $module }}</span>
                                                    {{ $config['label'] }}
                                                </td>
                                                @foreach ($allActions as $action)
                                                    @php
                                                        ...
                                                        $fromRole =
                                                            $role && $role->permissions->contains('name', $permName);
                                                    @endphp
                                                        ...
                                                        @if ($hasAction)
                                                            <div class="position-relative d-inline-block">
                                                                ...
                                                                @if ($user->hasRole('admin') || ($user->roles->first() && $user->roles->first()->hasPermissionTo($permName)))
                                                                    <span title="Đã có từ Role"
                                                                        style="position:absolute;top:-6px;right:-8px;font-size:.6rem;color:var(--primary)">
                            ...
                            <div class="d-flex gap-3 mb-3" style="font-size:.8rem">
                                <span><i class="fa-solid fa-circle-check text-primary me-1"></i>= Đã có từ Role (luôn
                                    active)</span>
                                <span><i class="fa-solid fa-square-check text-success me-1"></i>= Quyền riêng bổ
                                    sung</span>
                            </div>
    ...
    <style>
        ...
        .row-check {
            ...
            width: 1.1rem;
            height: 1.1rem;
            accent-color: var(--primary, #f7941d);
        }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelectorAll('.row-check').forEach(function(rowCb) {
                rowCb.addEventListener('change', function() {
                    ...
                    document.querySelectorAll('.perm-row-' + mod).forEach(cb => cb.checked = this
                        .checked);
                });
            });
            ...
            document.getElementById('btnClearPerms')?.addEventListener('click', function() {
                if (!confirm('Xóa toàn bộ quyền riêng của user này? (Quyền từ role vẫn giữ nguyên)'))
            return;
                document.querySelectorAll('.perm-check').forEach(c => c.checked = false);
