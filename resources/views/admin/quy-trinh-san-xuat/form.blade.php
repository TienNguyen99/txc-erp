...
@section('css')
    ...
    <style>
        ...
        .drag-drawflow:hover {
            border-color: #f7941d;
            box-shadow: 0 4px 6px -1px rgba(247, 148, 29, 0.1);
        }
...
@section('content')
    <div class="container-fluid px-4">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                <h4 class="page-title mb-1"><i
                        class="fa-solid fa-diagram-project me-2"></i>{{ isset($quyTrinh) ? 'Chỉnh sửa Quy trình' : 'Thêm mới Quy trình' }}
                </h4>
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-0">
                        <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">Dashboard</a></li>
                        <li class="breadcrumb-item"><a href="{{ route('admin.quy-trinh-san-xuat.index') }}">Quy trình Sản
                                xuất</a></li>
        ...
        <form id="quyTrinhForm"
            action="{{ isset($quyTrinh) ? route('admin.quy-trinh-san-xuat.update', $quyTrinh->id) : route('admin.quy-trinh-san-xuat.store') }}"
            method="POST">
            @csrf
            @if (isset($quyTrinh))
                @method('PUT')
                ...
                <div class="card-body">
                    <div class="row g-3">
                        <div class="col-md-3">
                            ...
                            <input type="text" class="form-control" name="ma_quy_trinh"
                                value="{{ old('ma_quy_trinh', $quyTrinh->ma_quy_trinh ?? '') }}" required>
                        </div>
                        <div class="col-md-5">
                            <label class="form-label">Tên Quy trình <span class="text-danger">*</span></label>
                            <input type="text" class="form-control" name="ten_quy_trinh"
                                value="{{ old('ten_quy_trinh', $quyTrinh->ten_quy_trinh ?? '') }}" required>
                        </div>
                        <div class="col-md-2">
                            <label class="form-label">Ngày hiệu lực</label>
                            <input type="date" class="form-control" name="ngay_hieu_luc"
                                value="{{ old('ngay_hieu_luc', isset($quyTrinh) && $quyTrinh->ngay_hieu_luc ? $quyTrinh->ngay_hieu_luc->format('Y-m-d') : '') }}">
                        </div>
                        <div class="col-md-2">
                            <label class="form-label">Trạng thái</label>
                            <select class="form-select" name="trang_thai">
                                <option value="active"
                                    {{ old('trang_thai', $quyTrinh->trang_thai ?? '') == 'active' ? 'selected' : '' }}>Đang
                                    sử dụng</option>
                                <option value="inactive"
                                    {{ old('trang_thai', $quyTrinh->trang_thai ?? '') == 'inactive' ? 'selected' : '' }}>
                        ...
                        <div class="col-md-12">
                            ...
                            <select class="form-select" name="san_pham_ap_dung[]" multiple id="san_pham_select">
                                ...
                                @foreach ($products as $p)
                                    <option value="{{ $p->ma_hh }}"
                                        {{ in_array($p->ma_hh, $selectedProducts) ? 'selected' : '' }}>{{ $p->ma_hh }}
                                        - {{ $p->ten_hh }}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="drawflow-wrapper">
                        <div class="drawflow-sidebar">
                            <h6 class="text-muted mb-3"
                                style="font-size: 12px; text-transform: uppercase; letter-spacing: 1px;">Danh sách Công đoạn
                            </h6>
...
@section('scripts')
    ...
    <script>
        ...
        function drop(ev) {
            if (ev.type === "touchend") {
                let parentdrawflow = document.elementFromPoint(mobile_last_move.touches[0].clientX, mobile_last_move
        ...
        function addNodeToDrawFlow(name, pos_x, pos_y) {
            ...
            switch (name) {
                ...
                case 'dinh_hinh':
                    ...
                    html =
                        `<div><div class="title-box">${icon} ${title}</div><div class="box">Công đoạn Định hình</div></div>`;
                ...
                case 'in_nhuoc':
                    ...
                    html =
                        `<div><div class="title-box">${icon} ${title}</div><div class="box">Công đoạn In/Nhuộm</div></div>`;
                ...
                case 'kcs':
                    ...
                    html =
                        `<div><div class="title-box">${icon} ${title}</div><div class="box">Kiểm tra chất lượng</div></div>`;
                ...
                case 'dong_goi':
                    ...
                    html =
                        `<div><div class="title-box">${icon} ${title}</div><div class="box">Công đoạn Đóng gói</div></div>`;
                ...
                case 'nhap_kho':
                    ...
                    html =
                        `<div><div class="title-box">${icon} ${title}</div><div class="box">Nhập kho thành phẩm</div></div>`;
                    editor.addNode('nhap_kho', 1, 0, pos_x, pos_y, 'nhap_kho', {},
                    html); // 1 input, 0 output for the end node
                    break;
            }
        }
